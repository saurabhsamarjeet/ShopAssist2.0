...
# Function to initialize the conversation with OpenAI model by setting the system's initial prompt
def initialize_conversation():
    '''
    Initializes the conversation with OpenAI by setting a system prompt.
    Returns:
...
# Function to send a chat completion request to OpenAI with the given messages
...
def intent_confirmation_layer(response_assistant):
    '''
    Verifies whether the assistant's response correctly includes all necessary laptop attributes.
    Args:
        response_assistant (str): Assistant's response.
    Returns:
        str: "Yes" if all values are captured correctly, otherwise "No".
    '''
    delimiter = "####"
    prompt = f"""
